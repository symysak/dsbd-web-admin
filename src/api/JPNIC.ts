...
import {JPNICRegistrationData, JPNICReturnData, JPNICSearchData} from "../interface";
...
export function ReturnAddress(data: JPNICReturnData): Promise<{ error: string; data: any }> {
    return axios.post(restfulApiConfig.apiURL + "/jpnic", data, {
        headers: {
            'Content-Type': 'application/json',
            ACCESS_TOKEN: sessionStorage.getItem('AccessToken'),
...
export function GetHandle(handle: string): Promise<{ error: string; data: any }> {
...
export function GetAll(data: JPNICSearchData): Promise<{ error: string; data: any }> {
